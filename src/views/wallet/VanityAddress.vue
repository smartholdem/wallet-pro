    ...
    <card>
      ...
      <div class="card-body">
        <div class="row">
          ...
          <div class="col-lg-6">
            ...
            <form @submit.prevent="startGeneration">
              <div class="mb-3">
                ...
                <input type="text" class="form-control" id="searchTerm" v-model="term" :disabled="isGenerating"
                       required>
              </div>
              ...
              <div class="mb-3">
                ...
                <div class="form-check">
                  <input class="form-check-input" type="radio" id="prefix" value="prefix" v-model="mode"
                         :disabled="isGenerating">
                  <label class="form-check-label" for="prefix">{{
                      $t('vanity_address_mode_prefix', {term: term})
                    }}</label>
                </div>
                <div class="form-check">
                  <input class="form-check-input" type="radio" id="contains" value="contains" v-model="mode"
                         :disabled="isGenerating">
                  <label class="form-check-label" for="contains">{{
                      $t('vanity_address_mode_contains', {term: term})
                    }}</label>
                </div>
                <div class="form-check">
                  <input class="form-check-input" type="radio" id="suffix" value="suffix" v-model="mode"
                         :disabled="isGenerating">
                  <label class="form-check-label" for="suffix">{{
                      $t('vanity_address_mode_suffix', {term: term})
                    }}</label>
                </div>
              </div>
              ...
              <div class="mb-3">
                ...
                <input type="number" class="form-control" id="threads" v-model.number="threads" :max="maxThreads"
                       min="1" :disabled="isGenerating">
              ...
              <button type="submit" class="btn btn-primary" :disabled="isGenerating">
                <span v-if="isGenerating"><i class="fas fa-spinner fa-spin"></i> {{
                    $t('vanity_address_generating')
                  }}</span>
          ...
          <div class="col-lg-6">
            <h5 v-show="!isExtension">{{ $t('vanity_address_result') }}</h5>
            <div v-if="!isGenerating && !foundWallet.address" class="alert alert-info mt-2">
            ...
            <div v-if="isGenerating" class="p-3 border rounded bg-dark">
              ...
              <p class="mb-0"><strong>{{ $t('vanity_address_total_attempts') }}</strong>
                {{ totalAttempts.toLocaleString() }}</p>
              <p class="mb-0"><strong>{{ $t('vanity_address_speed') }}</strong> {{ speed.toLocaleString() }}
                {{ $t('vanity_address_speed_unit') }}</p>
            </div>
            <div v-if="foundWallet.address" class="alert alert-success">
              ...
              <p class="mb-0"><strong>{{ $t('vanity_address_secret_phrase') }}</strong><br><code
                  class="text-danger">{{ foundWallet.secret }}</code></p>
              <hr>
